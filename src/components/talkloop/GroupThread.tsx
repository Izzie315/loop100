...
import { durationLabel, timeLabel } from "@/lib/talkloop";
...
type Reaction = { note_id: string; account_id: string; reaction: string };
...
const COLUMNS =
  "id, group_id, author_id, body, created_at, edited_at, hidden_for, media_url, media_kind, media_seconds";
...
export function GroupThread({
  ...
  const [selected, setSelected] = useState<LoopNote | null>(null);
  const [editing, setEditing] = useState<LoopNote | null>(null);
    ...
    const channel = supabase
      ...
      .on(
        "postgres_changes",
        {
          ...
          schema: "public",
          table: "group_notes",
          filter: `group_id=eq.${loopId}`,
        },
        (payload) => {
          const row = payload.new as LoopNote;
      ...
      .on(
        "postgres_changes",
        { event: "INSERT", schema: "public", table: "group_note_reactions" },
        (payload) => {
      ...
      .on(
        "postgres_changes",
        { event: "UPDATE", schema: "public", table: "group_note_reactions" },
        (payload) => {
          const row = payload.new as Reaction;
          setReactions((prev) =>
            prev.map((r) =>
      ...
      .on(
        "postgres_changes",
        { event: "DELETE", schema: "public", table: "group_note_reactions" },
        (payload) => {
          const gone = payload.old as { note_id?: string; account_id?: string };
          if (!gone?.note_id) return;
          setReactions((prev) =>
            prev.filter((r) => !(r.note_id === gone.note_id && r.account_id === gone.account_id)),
          );
        },
      )
      .subscribe();
  ...
  const onPick = (file: File | undefined) => {
    ...
    clearPending();
    setPending({ file, kind: isVideo ? "video" : "photo", previewUrl: URL.createObjectURL(file) });
  };
  ...
  const deliver = async () => {
    ...
    if (data) {
      setNotes((prev) =>
        prev.some((n) => n.id === (data as LoopNote).id) ? prev : [...prev, data as LoopNote],
      );
    }
    setBusy(false);
  };
  ...
  const beginPress = (note: LoopNote) => {
  ...
  const unsend = async (note: LoopNote) => {
    setSelected(null);
    setNotes((prev) => prev.filter((n) => n.id !== note.id));
    if (note.media_url) await supabase.storage.from("note-media").remove([note.media_url]);
    const { error } = await supabase.from("group_notes").delete().eq("id", note.id);
    if (error) toast.error("That note could not be pulled back.");
  };

  const hideForMe = async (note: LoopNote) => {
    setSelected(null);
    if (!meId) return;
  ...
  const react = async (note: LoopNote, key: string) => {
    setSelected(null);
    if (!meId) return;
    setReactions((prev) => [
  ...
  const dropReaction = async (note: LoopNote) => {
    setSelected(null);
    if (!meId) return;
  ...
  return (
    <div className="flex h-full min-h-0 flex-col">
      <div className="flex items-center gap-2 pb-1">
        ...
        <h2 className="truncate text-base font-semibold">{loopName}</h2>
      </div>
      ...
      <Dialog open={!!selected} onOpenChange={(open) => !open && setSelected(null)}>
        <DialogContent className="max-w-xs">
            ...
            <div className="grid grid-cols-5 gap-1">
              {REACTIONS.map((r) => {
                const active = reactions.some(
                  (x) => x.note_id === selected.id && x.account_id === meId && x.reaction === r.key,
                );
                return (
                  <button
                    key={r.key}
          ...
          <div className="flex flex-col gap-2">
            ...
            {selected &&
              reactions.some((x) => x.note_id === selected.id && x.account_id === meId) && (
                <Button
                  variant="secondary"
                  className="justify-start"
